...
class PEDraftAssesmentFinalize: BaseViewController , DatePickerPopupViewControllerProtocol {
    ...
    let refrigeratorNitrogenStr = Constants.refrigeratorNitrogenStr
    ...
    fileprivate func handleCatArrForIdsDoaValidation() {
        if regionID != 3 {
    ...
    fileprivate func handleCatArrayForCollectionIsDoasValidation() {
        for cat in catArrayForCollectionIs {
            if cat.doaS.count > 0 {
                var idArr : [Int] = []
                for obj in  cat.doaS {
    ...
    fileprivate func handleCatArrayForCollectionIsInovoject() {
        for cat in catArrayForCollectionIs {
    ...
    fileprivate func handleCatArrayForCollectionIsVMixer() {
        for cat in catArrayForCollectionIs {
    ...
    fileprivate func handleExtendedPESaveExtendedMicrobialInLocalDB() {
        if regionID == 3 {
            if showExtendedPE {
    ...
    fileprivate func handleAndManageCatIdEvaluationDateValidation(_ row: inout Int) {
        for cat in catArrayForCollectionIs {
    ...
    override func viewDidLoad() {
        ...
        handleCatArrForIdsDoaValidation()
        handleCatArrayForCollectionIsDoasValidation()
        handleCatArrayForCollectionIsInovoject()
        handleCatArrayForCollectionIsVMixer()
        ...
        handleAndManageCatIdEvaluationDateValidation(&row)
        ...
        handleExtendedPESaveExtendedMicrobialInLocalDB()
    ...
    fileprivate func showAlertOnDescriptionValidation(_ question: PE_ExtendedPEQuestion) {
    ...
    fileprivate func handleShowExtendedPEValidateForm() -> Bool? {
        if showExtendedPE{
            ...
            for question in sanitationQuesArr {
                ...
                showAlertOnDescriptionValidation(question)
	...
	fileprivate func handleRegionIdAndManageStrings() {
		if regionID == 3, strings.contains(Constants.pleaseEnterVaccineDetailsStr) {
			strings.removeAll { $0 == Constants.pleaseEnterVaccineDetailsStr }
		}
	}
	
	private func handleInovojectDataAndShowAlerts() -> Bool? {
        guard !inovojectData.isEmpty else {
            showAlertForNoValid()
            return regionID == 3 ? nil : false
        }
        
        let firstNameCount = inovojectData[0].name?.count ?? 0
        if firstNameCount < 1 {
            showAlertForNoValid()
            return regionID == 3 ? nil : false
        }

		handleRegionIdAndManageStrings()
		
        for inovo in inovojectData {
            if inovo.invoHatchAntibiotic == 1, inovo.invoHatchAntibioticText.isEmpty {
        ...
        if peNewAssessment.hatcheryAntibioticsDoa == 1,
           peNewAssessment.hatcheryAntibioticsDoaText?.isEmpty ?? true {
            showAlertForAntibiotic()
            return regionID == 3 ? nil : false
        }

        if peNewAssessment.hatcheryAntibioticsDoaS == 1,
           peNewAssessment.hatcheryAntibioticsDoaSText?.isEmpty ?? true {
            showAlertForAntibiotic()
            return regionID == 3 ? nil : false
        }
    ...
    func handleCertificateDataShowAlerts() -> Bool? {
    ...
    private func handleRegionIDAndFrequencyValidations() -> Bool? {
        if regionID == 3 {
    ...
    private func handlePENewAssessmentQCCountAndValidations() -> Bool? {
    ...
    func validateForm() -> Bool {
        ...
        if let boolVal = handleShowExtendedPEValidateForm() {
            return boolVal
        }
        if !(self.peNewAssessment.evaluationName?.contains("Non") ?? false),
           let status = handleInovojectDataAndShowAlerts() {
            return status
        }
        
        if self.checkForTraning() && !(self.peNewAssessment.evaluationName?.contains("Non") ?? false),let status = handleCertificateDataShowAlerts() {
            return status
        }
        if self.checkForTraning() {
            if let status = handleRegionIDAndFrequencyValidations() {
                return status
            }
            
            if let status = handlePENewAssessmentQCCountAndValidations() {
                return status
            }
        }
    ...
    func showAlertForAntibiotic() {
        if regionID == 3
        {
            ...
            let errorMSg = Constants.pleaseEnterAntibiotics
            let alertController = UIAlertController(title: "Alert", message: errorMSg, preferredStyle: .alert)
            let okAction = UIAlertAction(title: "OK", style: UIAlertAction.Style.default)
            let cancelAction = UIAlertAction(title: "Cancel", style: UIAlertAction.Style.cancel)
            alertController.addAction(okAction)
    ...
    fileprivate func handleRegionIDAndOtherValidationsFinalizeButtonClicked() {
        if regionID == 3 {
            if strings.count > 0 {
                let storyBoard : UIStoryboard = UIStoryboard(name: "PEStoryboard", bundle:nil)
    ...
    fileprivate func handleValidateFormAndRegionIdValidationsFinalizeButtonClicked() {
        if regionID == 3 {
            if strings.count > 0 {
                let storyBoard : UIStoryboard = UIStoryboard(name: "PEStoryboard", bundle:nil)
                let vc = storyBoard.instantiateViewController(withIdentifier: "customAlertView") as! customAlertView
        ...
        if regionID == 3 {
            if peNewAssessment.isPERejected == false && peNewAssessment.isEMRejected == true {
                ...
                if checkNoteForEveryQuestion(),validateForm() {
                    self.handleRegionIDAndOtherValidationsFinalizeButtonClicked()
                }
            }
        } else {
            finishingAssessment = true
            ...
            if checkNoteForEveryQuestion() {
                if validateForm() {
                    handleValidateFormAndRegionIdValidationsFinalizeButtonClicked()
    ...
    fileprivate func saveUpdateOfflineFinlizeDataInLocalDB() {
    ...
    fileprivate func saveUpdateRefrigatorInLocalDB() {
        let refrigtorArray = CoreDataHandlerPE().getDraftREfriData(id: Int(self.selectedCategory?.serverAssessmentId ?? "0") ?? 0)
        if(refrigtorArray.count > 0) {
            for refrii in refrigtorArray {
    ...
    private func saveFinalizedData() {
        ...
        saveUpdateRefrigatorInLocalDB()
        saveUpdateOfflineFinlizeDataInLocalDB()
    ...
    func callExtendedMicro(param:JSONDictionary) {
        ...
        ZoetisWebServices.shared.sendExtendedMicroToServer(controller: self, parameters: param, completion: { [weak self] (json, error) in
            ...
            if json["StatusCode"] == 200 {
                ...
                selfObj.showtoast(message: Constants.dataSyncSuccess)
	...
	private func updateSpecialFields(for assessment: PE_AssessmentInProgress, isNA: Bool) {
		if assessment.sequenceNoo == 3 && assessment.rollOut == "Y" {
			...
			CoreDataHandlerPE().updateInDoGInProgressInDB(newAssessment: peNewAssessment, fromDraft: true)
		}
	...
	private func handleDispatchQueueTotalScoreLbl(_ totalresult: Int, _ result: Int) {
		DispatchQueue.main.async { [weak self] in
			guard let self = self else { return }
			self.totalScoreLabel.text = String(totalresult)
			self.resultScoreLabel.text = String(result)
	...
	private func handleDispatchQueueTotalResultText(_ totalresult: Int, _ result: Int) {
		DispatchQueue.main.async { [weak self] in
			guard let self = self else { return }
			self.totalScoreLabel.text = String(totalresult)
			self.resultScoreLabel.text = String(result)
...
extension PEDraftAssesmentFinalize: UITableViewDelegate, UITableViewDataSource{
    ...
    func numberOfSections(in tableView: UITableView) -> Int {
        guard catArrayForTableIs.count > 0 else {
            return sectionCountForEmptyCatArray()
        }
        
        if let assessment = catArrayForTableIs.first as? PE_AssessmentInProgress {
            return sectionCountForAssessment(assessment)
        }
        
        return sectionCountForCategoryFallback()
    }

    private func sectionCountForAssessment(_ assessment: PE_AssessmentInProgress) -> Int {
    ...
    private func sectionCountForCategoryFallback() -> Int {
    ...
    private func sectionCountForEmptyCatArray() -> Int {
        if selectedCategory?.sequenceNoo == 12,
           selectedCategory?.catName != Constants.refrigeratorNitrogenStr {
            return 1
        }
    ...
    private func getRowHeight(indexPath:IndexPath,assessment: PE_AssessmentInProgress?) -> Int? {
        if selectedCategory?.sequenceNoo == 3 {
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        if let rowHeight = getRowHeight(indexPath: indexPath, assessment: assessment) {
        ...
        if(selectedCategory?.catName != Constants.refrigeratorNitrogenStr),indexPath.section > 0 {
            return 130
        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if checkForTraning() {
            if isSanitationSection(indexPath) {
                return setupSanitationCell(tableView, cellForRowAt: indexPath)
            }
            ...
            if indexPath.section == 1,
               let cell = tableView.dequeueReusableCell(withIdentifier: VaccineMixerCell.identifier) as? VaccineMixerCell {
                configureVaccineMixerCell(cell, at: indexPath, tableView: tableView)
                return cell
            }
        }
        // Fallback/default cell if none of the above match
        return UITableViewCell()
    }

    // Helper 1: Check if this is the sanitation section
    private func isSanitationSection(_ indexPath: IndexPath) -> Bool {
        return indexPath.section == 0 &&
            selectedCategory?.sequenceNoo == 12 &&
            selectedCategory?.catName == Constants.extendedMicrobialStr
    }

    // Helper 2: Setup sanitation cell
    private func setupSanitationCell(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        setupSanitationPlateTypeCompletion(cell, indexPath: indexPath)
        setupSanitationCommentsCompletion(cell, indexPath: indexPath, tableView: tableView)
        self.refreshScore(indexPath.row)
        cell.assessmentId = peNewAssessment?.serverAssessmentId
        return cell
    }

    private func setupSanitationPlateTypeCompletion(_ cell: PlateInfoCell, indexPath: IndexPath) {
        cell.plateTypeCompletion = { [unowned self] error in
            ...
            self.dropDownVIewNew(arrayData: arr as? [String] ?? [String](), kWidth: cell.plateTypeBtn.frame.width, kAnchor: cell.plateTypeBtn, yheight: cell.plateTypeBtn.bounds.height) { [unowned self] selectedVal, index in
                if indexPath.row > -1 && self.sanitationQuesArr.count > indexPath.row {
                    var quesObj = self.sanitationQuesArr[indexPath.row]
    ...
    private func setupSanitationCommentsCompletion(_ cell: PlateInfoCell, indexPath: IndexPath, tableView: UITableView) {
    ...
    // Helper 3: Configure VaccineMixerCell
    private func configureVaccineMixerCell(_ cell: VaccineMixerCell, at indexPath: IndexPath,tableView:UITableView) {
        ...
        configureVaccineMixerCellUserInteraction(cell)
        configureVaccineMixerCellBorderColors(cell, at: indexPath)
        configureVaccineMixerCellCompletions(cell, at: indexPath,tableView: tableView)
    }

    private func configureVaccineMixerCellUserInteraction(_ cell: VaccineMixerCell) {
        if regionID == 3 {
            if peNewAssessment.isPERejected == false && peNewAssessment.isEMRejected == true {
    ...
    private func configureVaccineMixerCellBorderColors(_ cell: VaccineMixerCell, at indexPath: IndexPath) {
        if certificateData.count > 0 {
            cell.config(data: certificateData[indexPath.row])
	...
	fileprivate func handleRegionAndupdateCell(_ date: String?,_ cell: VaccineMixerCell) {
		if regionID != 3 {
	...
	private func configureVaccineMixerCellCompletions(_ cell: VaccineMixerCell, at indexPath: IndexPath,tableView:UITableView) {
        cell.certDateCompletion = { [unowned self] (count) in
            if cell.vaccNameField.text != "", !dataArray.contains(cell.vaccNameField.text!) {
				...
				handleRegionAndupdateCell(date,cell)
                cell.certDateSelectBtn.layer.borderColor = UIColor(red: 0.0, green: 200.0, blue: 226.0, alpha: 1.0).cgColor
        ...
        cell.changedDateCompletion = { [unowned self] (index) in
            ...
            if cell.vaccNameField.text != "", (cell.certDateSelectBtn.titleLabel?.text != "") {
                ...
                if let datePickerPopupViewController = datePickerPopupViewController {
        ...
        changedDate = { [unowned self] (date) in
            ...
            tableView.reloadRows(at: [self.tableviewIndexPath], with: .automatic)
        }
    }
    
    
    // MARK: - Setup Inovoject Cell in Table View
    fileprivate func handleDiluentManuCompletionSetupInvojectCellForRow(_ cell: InovojectNewTableViewCell, _ indexPath: IndexPath) {
    ...
    fileprivate func handleBagSizeCompletionSetupInvojectCellForRowSubMethod(_ c: Double, _ indexPath: IndexPath) {
    ...
    fileprivate func handleBagSizeCompletionSetupInvojectCellForRow(_ cell: InovojectNewTableViewCell, _ indexPath: IndexPath) {
        cell.bagSizeCompletion = {[unowned self] ( error) in
            ...
            if bagSizeArray.count > 0 {
                self.dropDownVIewNew(arrayData: bagSizeArray as? [String] ?? [String](), kWidth:  cell.tfBagSize.frame.width, kAnchor: cell.tfBagSize, yheight: cell.tfBagSize.bounds.height) { [unowned self] selectedVal, index  in
                    ...
                    handleBagSizeCompletionSetupInvojectCellForRowSubMethod(c,indexPath)
                    CoreDataHandlerPE().updateDOAInDB(inovojectData:  self.inovojectData[indexPath.row])
                    self.reloadTableViewWithoutAnimation()
                }
                self.dropHiddenAndShow()
    ...
    fileprivate func handleProgramAntibioicOtherCompletionHandlerSetupInvojectCellForow(_ cell: InovojectNewTableViewCell, _ indexPath: IndexPath) {
    ...
    fileprivate func handleAmpleSizeCompletionSetupInvojectCellForRow(_ cell: InovojectNewTableViewCell, _ indexPath: IndexPath) {
    ...
    fileprivate func handleAmplePerBagCompletionSetupInvojectCellForRowSubMethod(_ c: Double, _ indexPath: IndexPath) {
        let a = Double(self.inovojectData[indexPath.row].ampulePerBag ?? "0") ?? 0
        let b = Double(self.inovojectData[indexPath.row].ampuleSize ?? "0") ?? 0
        if b != 0 && a != 0 && b != 0{
            let x = a * b
    ...
    fileprivate func handleAmplePerBagCompletionSetupInvojectCellForRow(_ cell: InovojectNewTableViewCell, _ indexPath: IndexPath) {
        cell.amplePerBagCompletion = {[unowned self] ( error) in
            ...
            if vManufacutrerNameArray.count > 0 {
                self.dropDownVIewNew(arrayData: vManufacutrerNameArray as? [String] ?? [String](), kWidth: cell.tfAmpleBag.frame.width, kAnchor: cell.tfAmpleBag, yheight: cell.tfAmpleBag.bounds.height) { [unowned self] selectedVal, index  in
                    ...
                    handleAmplePerBagCompletionSetupInvojectCellForRowSubMethod(c,indexPath)
                    CoreDataHandlerPE().updateDOAInDB(inovojectData:  self.inovojectData[indexPath.row])
                    self.reloadTableViewWithoutAnimation()
                }
                self.dropHiddenAndShow()
    ...
    fileprivate func handleNameCompletionCetupInvojectCellForRow(_ cell: InovojectNewTableViewCell, _ indexPath: IndexPath) {
        cell.nameCompletion = {[unowned self] ( text) in
            ...
            if text != "" {
                ...
                if vNameFilterArray.count > 0 {
                    self.dropDownVIewNew(arrayData: vNameFilterArray, kWidth: cell.tfVaccineMan.frame.width, kAnchor: cell.tfVaccineMan, yheight: cell.tfVaccineMan.bounds.height) { [unowned self] selectedVal, index  in
    ...
    fileprivate func handleCellSetupInovojectCell(_ indexPath: IndexPath, _ cell: InovojectNewTableViewCell) {
    ...
    func setupInovojectCell(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> InovojectNewTableViewCell {
        if let cell =  tableView.dequeueReusableCell(withIdentifier: InovojectNewTableViewCell.identifier) as? InovojectNewTableViewCell {
            ...
            handleCellSetupInovojectCell(indexPath, cell)
            handleDiluentManuCompletionSetupInvojectCellForRow(cell, indexPath)
            handleBagSizeCompletionSetupInvojectCellForRow(cell, indexPath)
            handleProgramAntibioicOtherCompletionHandlerSetupInvojectCellForow(cell, indexPath)
            ...
            handleAmpleSizeCompletionSetupInvojectCellForRow(cell, indexPath)
            handleAmplePerBagCompletionSetupInvojectCellForRow(cell, indexPath)
            handleNameCompletionCetupInvojectCellForRow(cell, indexPath)
            return cell
        }
    ...
    fileprivate func handleNameCompletionEmptyTextCase(cell:InovojectCell,indexPath:IndexPath,_ text: String?) {
        if text != "" {
    ...
    func setupDayOfAgeCell(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> InovojectCell {
        ...
        if let cell = tableView.dequeueReusableCell(withIdentifier: InovojectCell.identifier) as? InovojectCell{
            ...
            cell.nameCompletion  = {[unowned self] ( text) in
                self.tableviewIndexPath = indexPath
                self.handleNameCompletionEmptyTextCase(cell: cell, indexPath: indexPath, text)
            }
    ...
    fileprivate func handleRefrigtorProbeArraySetupRefridgrateQuesCell(_ assesmentArray: inout [PE_AssessmentInProgress], _ indexPath: IndexPath, _ cell: RefrigatorQuesCell) {
    ...
    fileprivate func handleSetupRefridgerateDataBtnNaValidation(_ switchisCheck: Bool,_ indexPath:IndexPath,cell:RefrigatorQuesCell,assessment:PE_AssessmentInProgress?) {
        ...
        if(switchisCheck) {
            if(CoreDataHandlerPE().someDraftRefriEntityExists(id: assessment?.assID as! Int)) {
                ...
                let draftData = CoreDataHandlerPEModels.RefrigatorDraftData(
                      ...
                      isCheck: true,
                      isNA: false,
                      schAssmentId: Int(selectedCategory?.serverAssessmentId ?? "0") ?? 0
                )
    ...
    fileprivate func handleSetupRefrigderateValidationBtnSelectedFalse(_ switchisCheck: Bool,assessment:PE_AssessmentInProgress?) {
        if(switchisCheck) {
            if(CoreDataHandlerPE().someDraftRefriEntityExists(id: assessment?.assID as! Int)) {
    ...
    fileprivate func handleCameraCompletion(_ cell: RefrigatorQuesCell, _ indexPath: IndexPath, _ assesmentArray: [PE_AssessmentInProgress]) {
    ...
    fileprivate func handleCompletionCellIndexPathAssessmentSetupRefridgerator(_ cell: RefrigatorQuesCell, _ indexPath: IndexPath, _ assessment: PE_AssessmentInProgress?) {
                ...
                if status ?? false {
                    ...
                    if(CoreDataHandlerPE().someDraftRefriEntityExists(id: assessment?.assID as! Int)) {
                        ...
                        let draftData = CoreDataHandlerPEModels.RefrigatorDraftData(
                              ...
                              isCheck: false,
                              isNA: true,
                              schAssmentId: Int(self.selectedCategory?.serverAssessmentId ?? "0") ?? 0
                        )
    ...
    fileprivate func handleCommentCompletionSetupRefrigderator(_ cell: RefrigatorQuesCell, _ indexPath: IndexPath, _ assessment: PE_AssessmentInProgress?) {
    ...
    fileprivate func handlebtnNAValidationSwitchCheckUpdates(_ switchisCheck: inout Bool,_ indexPath:IndexPath,assesmentArray:[PE_AssessmentInProgress]) {
    ...
    fileprivate func handleUIUpdatesAndOtherValidations(_ cell: RefrigatorQuesCell, _ assesmentArray: inout [PE_AssessmentInProgress], _ indexPath: IndexPath, _ assessment: PE_AssessmentInProgress?) {
    ...
    func setUpRerigatorQuesCell(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> RefrigatorQuesCell {
        ...
        if let cell = tableView.dequeueReusableCell(withIdentifier: RefrigatorQuesCell.identifier) as? RefrigatorQuesCell{
            ...
            var assessment = assesmentArray[indexPath.row] as? PE_AssessmentInProgress
            if(refrigtorProbeArray.count > 0) {
                handleRefrigtorProbeArraySetupRefridgrateQuesCell(&assesmentArray, indexPath, cell)
            }
            
            handleUIUpdatesAndOtherValidations(cell, &assesmentArray, indexPath, assessment)
            ...
            cell.btnNA = {[unowned self] () in
                ...
                handlebtnNAValidationSwitchCheckUpdates(&switchisCheck,indexPath,assesmentArray: assesmentArray)
                if(cell.btn_NA.isSelected) {
                    handleSetupRefridgerateDataBtnNaValidation(switchisCheck, indexPath, cell: cell, assessment: assessment)
                } else {
                    ...
                    handleSetupRefrigderateValidationBtnSelectedFalse(switchisCheck, assessment: assessment)
            ...
            handleCameraCompletion(cell, indexPath, assesmentArray)
            ...
            handleCompletionCellIndexPathAssessmentSetupRefridgerator(cell, indexPath, assessment)
            handleCommentCompletionSetupRefrigderator(cell, indexPath, assessment)
            return cell
        }
    ...
    fileprivate func handlePENewAssessmentDDt() {
    ...
    fileprivate func handleVaccineManufacturerCompletionSetupDayOfAgeCell(_ cell: InovojectCell, _ indexPath: IndexPath) {
        cell.vaccineManufacturerCompletion = {[unowned self] ( error) in
            self.tableviewIndexPath = indexPath
    ...
    fileprivate func handleAmpleSizeCompletionSetupDayeOfAgeCell(_ cell: InovojectCell, _ indexPath: IndexPath) {
        cell.ampleSizeCompletion = {[unowned self] ( error) in
            self.tableviewIndexPath = indexPath
    ...
    fileprivate func handleAmplePerBagCompletionSetupDayOfAgeCell(_ cell: InovojectCell, _ indexPath: IndexPath) {
        cell.amplePerBagCompletion = {[unowned self] ( error) in
            self.tableviewIndexPath = indexPath
    ...
    fileprivate func handleNameCompletionSetupDayOfAgeCell(_ cell: InovojectCell, _ indexPath: IndexPath) {
        cell.nameCompletion = {[unowned self] ( text) in
            self.tableviewIndexPath = indexPath
            if text != "" {
    ...
    func setupDayOfAgeSCell(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> InovojectCell {
        ...
        if let cell = tableView.dequeueReusableCell(withIdentifier: InovojectCell.identifier) as? InovojectCell {
            ...
            handleVaccineManufacturerCompletionSetupDayOfAgeCell(cell, indexPath)
            handleAmpleSizeCompletionSetupDayeOfAgeCell(cell, indexPath)
            handleAmplePerBagCompletionSetupDayOfAgeCell(cell, indexPath)
            handleNameCompletionSetupDayOfAgeCell(cell, indexPath)
    ...
    fileprivate func handleTextfieldTextForResultAndTotalResult(_ result: Int, _ totalresult: Int) {
        DispatchQueue.main.async {
            self.resultScoreLabel.text = String(result)
            self.totalScoreLabel.text = String(totalresult)
        }
    ...
    private func configureNAButton(for cell: PEQuestionTableViewCell, assessment: PE_AssessmentInProgress?) {
        ...
        cell.btn_NA.isUserInteractionEnabled = !btn_NA.isSelected
    }
    ...
    func tableView(_ tableView: UITableView, viewForFooterInSection section: Int) -> UIView? {
        guard isRefrigeratorFooterSection() else { return UIView() }
        ...
        switch section {
        case 0:
				return setupFirstFooterSection(tableView, array: array as! [Any])
        case 1:
				return setupSecondFooterSection(tableView, array: array as! [Any])
        default:
            return setupDefaultFooterSection(tableView)
        }
    }
    
    private func isRefrigeratorFooterSection() -> Bool {
        return selectedCategory?.sequenceNoo == 11 && selectedCategory?.catName == Constants.refrigeratorNitrogenStr
    }
    
    private func setupFirstFooterSection(_ tableView: UITableView, array: [Any]) -> UIView? {
        ...
        configureFirstFooterSection(footerView)
        setupFirstFooterSectionData(footerView, array: array)
        setupFirstFooterSectionCompletions(footerView, array: array)
        return footerView
    }
    
    private func configureFirstFooterSection(_ footerView: RefrigatorTempProbeCell) {
        ...
        resetFooterTextFields(footerView)
        updateFooterAlpha(footerView)
    }
    
    private func resetFooterTextFields(_ footerView: RefrigatorTempProbeCell) {
        footerView.topValueTxtFld.text = ""
        footerView.middleValueTxtFld.text = ""
        footerView.bottomValueTxtFld.text = ""
    }
    
    private func updateFooterAlpha(_ footerView: RefrigatorTempProbeCell) {
        footerView.contentView.alpha = (btn_NA.isSelected && self.selctedNACategoryArray.contains(78)) ? 0.3 : 1
    }
    
    private func setupFirstFooterSectionData(_ footerView: RefrigatorTempProbeCell, array: [Any]) {
        guard refrigtorProbeArray.count > 0 else { return }
        
        for i in 2...4 {
            guard let ar = array[i] as? PE_AssessmentInProgress else { continue }
            for j in 0..<refrigtorProbeArray.count-1 {
                guard ar.assID == refrigtorProbeArray[j].id else { continue }
                updateFooterField(footerView, index: i, probe: refrigtorProbeArray[j])
            }
        }
    ...
    private func updateFooterField(_ footerView: RefrigatorTempProbeCell, index: Int, probe: PE_Refrigators) {
        switch index {
        case 2:
    ...
    private func updateMainUnitField(_ footerView: RefrigatorTempProbeCell) {
        for probe in refrigtorProbeArray {
            if let unit = probe.unit, !unit.isEmpty {
    ...
    private func setupFirstFooterSectionCompletions(_ footerView: RefrigatorTempProbeCell, array: [Any]) {
        setupMainTempUnitCompletion(footerView)
        setupUnitCompletion(footerView, array: array)
        setupValueCompletion(footerView, array: array)
        footerView.setGraddientAndLayerQcCountextFieldView()
    }
    
    private func setupMainTempUnitCompletion(_ footerView: RefrigatorTempProbeCell) {
        footerView.mainTempUnitCompletion = { [weak self] sender, txtfld, textLabel in
            guard let self = self else { return }
            ...
            if !unitArray.isEmpty {
                self.dropDownVIewNew(arrayData: unitArray, kWidth: (sender ?? UIButton()).frame.width, kAnchor: sender ?? UIButton(), yheight: (sender ?? UIButton()).bounds.height) { selectedVal, index in
                    ...
                    self.refrigtorProbeArray = CoreDataHandlerPE().getDraftREfriData(id: Int(self.selectedCategory?.serverAssessmentId ?? "0") ?? 0)
                    self.tableview.reloadData()
                }
                self.dropHiddenAndShow()
            }
    ...
    private func setupUnitCompletion(_ footerView: RefrigatorTempProbeCell, array: [Any]) {
        footerView.unitCompletion = { [weak self] sender, txtfld, textLabel in
            guard let self = self else { return }
            let unitArray = ["Fahrenheit", "Celsius"]
            if !unitArray.isEmpty {
                self.handleUnitSelection(sender, txtfld, textLabel, array: array, footerView: footerView)
                self.dropHiddenAndShow()
            }
        }
    }
    
    private func handleUnitSelection(_ sender: UIButton?, _ txtfld: UITextField, _ textLabel: String, array: [Any], footerView: RefrigatorTempProbeCell) {
        self.dropDownVIewNew(arrayData: ["Fahrenheit", "Celsius"], kWidth: (sender ?? UIButton()).frame.width, kAnchor: sender ?? UIButton(), yheight: (sender ?? UIButton()).bounds.height) { [weak self] selectedVal, index in
            guard let self = self else { return }
            txtfld.text = selectedVal
            
            let (assessment, valueText) = self.getAssessmentAndValueText(textLabel, array: array, footerView: footerView)
    ...
    private func getAssessmentAndValueText(_ textLabel: String, array: [Any], footerView: RefrigatorTempProbeCell) -> (PE_AssessmentInProgress?, String) {
    ...
    private func setupValueCompletion(_ footerView: RefrigatorTempProbeCell, array: [Any]) {
        footerView.valueCompletion = { [weak self] value, textLabel in
            guard let self = self else { return }
            self.handleValueCompletion(value, textLabel, array: array, footerView: footerView)
        }
    }
    
    private func handleValueCompletion(_ value: UITextField?, _ textLabel: String, array: [Any], footerView: RefrigatorTempProbeCell) {
        let (unitValue, assessment) = getUnitValueAndAssessment(textLabel, array: array, footerView: footerView)
        let assID = assessment?.assID
        ...
        if CoreDataHandlerPE().someDraftRefriEntityExists(id: assID as! Int) {
            ...
            let draftData = CoreDataHandlerPEModels.updateDraftRefrigeratorData(
                ...
                rollOut: "Y",
                unit: unitValue,
                value: Double(value?.text ?? "0.0") ?? 0.0,
                catID: self.selectedCategory?.catID as! NSNumber,
                isCheck: true,
                isNA: false,
                serverAssessmentId: Int(self.selectedCategory?.serverAssessmentId ?? "0") ?? 0
            )
    ...
    private func getUnitValueAndAssessment(_ textLabel: String, array: [Any], footerView: RefrigatorTempProbeCell) -> (String, PE_AssessmentInProgress?) {
        var unitValue = ""
        var assessment: PE_AssessmentInProgress?
        ...
        return (unitValue, assessment)
    }
    
    private func setupSecondFooterSection(_ tableView: UITableView, array: [Any]) -> UIView? {
        let footerView = tableView.dequeueReusableHeaderFooterView(withIdentifier: RefrigatorTempProbeCell.identifier) as! RefrigatorTempProbeCell
        configureSecondFooterSection(footerView)
        setupSecondFooterSectionData(footerView, array: array)
        setupSecondFooterSectionCompletions(footerView, array: array)
        return footerView
    }
    
    private func configureSecondFooterSection(_ footerView: RefrigatorTempProbeCell) {
    ...
    private func setupSecondFooterSectionData(_ footerView: RefrigatorTempProbeCell, array: [Any]) {
        guard refrigtorProbeArray.count > 0 else { return }
        
        for i in 7...9 {
            guard let ar = array[i] as? PE_AssessmentInProgress else { continue }
            for j in 0..<refrigtorProbeArray.count {
                guard ar.assID == refrigtorProbeArray[j].id else { continue }
                updateFooterField(footerView, index: i, probe: refrigtorProbeArray[j])
            }
        }
    ...
    private func setupSecondFooterSectionCompletions(_ footerView: RefrigatorTempProbeCell, array: [Any]) {
        setupUnitCompletion(footerView, array: array)
        setupValueCompletion(footerView, array: array)
        footerView.setGraddientAndLayerQcCountextFieldView()
    }
    
    private func setupDefaultFooterSection(_ tableView: UITableView) -> UIView? {
        ...
        updateFrezerFooterAlpha(footerView)
        setupDefaultFooterSectionData(footerView)
        return footerView
    }
    
    private func updateFrezerFooterAlpha(_ footerView: FrezerFooterViewCell) {
        footerView.contentView.alpha = (btn_NA.isSelected && self.selctedNACategoryArray.contains(78)) ? 0.3 : 1
    }
    
    private func setupDefaultFooterSectionData(_ footerView: FrezerFooterViewCell) {
        footerView.textFieldView.text = self.peNewAssessment.refrigeratorNote ?? ""
        footerView.noteCompletion = { [weak self] textLabel in
            guard let self = self else { return }
    ...
    private func setupSanitationHeader(_ tableView: UITableView) -> UIView? {
        ...
        headerView.contentView.alpha = btn_NA.isSelected ? 0.3 : 1.0
        return headerView
    }
    ...
    private func setupRefrigeratorHeader(_ tableView: UITableView) -> UIView? {
        ...
        headerView.contentView.alpha = (btn_NA.isSelected && self.selctedNACategoryArray.contains(78)) ? 0.3 : 1.0
    ...
    private func setupRefrigeratorHeaderUnitCompletion( _ headerView: SetFrezzerPointCell,array: [Any],assessment: PE_AssessmentInProgress?,unitValue: String,valueText: String) {
    ...
    private func setupVaccineMixerHeader(_ tableView: UITableView) -> UIView? {
        ...
        if regionID == 3 {
            if !(peNewAssessment.isPERejected == false && peNewAssessment.isEMRejected == true) {
                setupVaccineMixerHeaderCompletions(headerView)
            }
        } else {
            setupVaccineMixerHeaderCompletions(headerView)
        }
    ...
    private func setupVaccineMixerHeaderCompletions(_ headerView: PETableviewHeaderFooterView) {
        headerView.addCompletion = { [unowned self] error in
            if self.certificateData.last == nil || self.certificateData.last?.name != "" {
    ...
    fileprivate func handleMinusCompletionAndHandleUI(_ headerView: PEInovojectHeaderFooterView) {
        headerView.minusCompletion = {[unowned self] ( error) in
    ...
    fileprivate func handleCSizeCompletionAndLocalDBUpdate(_ headerView: PEInovojectHeaderFooterView, _ section: Int) {
    ...
    func setPEInovojectHeaderFooterView(_ tableView: UITableView , section:Int) -> PEInovojectHeaderFooterView {
        if selectedCategory?.sequenceNoo == 1 {
            ...
            handleMinusCompletionAndHandleUI(headerView)
            ...
            handleCSizeCompletionAndLocalDBUpdate(headerView, section)
            return headerView
        } else {
    ...
    fileprivate func handleAddCompletionSetPEHeaderDayOfAge(_ headerView: PEHeaderDayOfAge) {
        headerView.addCompletion = {[unowned self] ( error) in
    ...
    fileprivate func handleMinusCompletionSetPEHeaderDayOfAge(_ headerView: PEHeaderDayOfAge) {
        headerView.minusCompletion = {[unowned self] ( error) in
    ...
    fileprivate func handleDTypeCompletionSetPEHeaderDayOfAge(_ headerView: PEHeaderDayOfAge) {
        headerView.dTypeCompletion = {[unowned self] ( error) in
    ...
    fileprivate func handleCSizeCompletionSetPEHeaderDayOfAge(_ headerView: PEHeaderDayOfAge) {
        headerView.cSizeCompletion = {[unowned self] ( error) in
    ...
    func setPEHeaderDayOfAge(_ tableView: UITableView , section:Int) -> PEHeaderDayOfAge {
        if selectedCategory?.sequenceNoo == 1 {
            ...
            let ispeRejectedEmRejected = peNewAssessment.isPERejected == false && peNewAssessment.isEMRejected == true
            
            if regionID == 3 {
                headerView.btn1.isUserInteractionEnabled = !ispeRejectedEmRejected
                headerView.switchHatchery.isUserInteractionEnabled = !ispeRejectedEmRejected
                headerView.btn2.isUserInteractionEnabled = !ispeRejectedEmRejected
                headerView.actionMinus.isUserInteractionEnabled = !ispeRejectedEmRejected
                headerView.actionAdd.isUserInteractionEnabled = !ispeRejectedEmRejected
                headerView.txtDType.isUserInteractionEnabled = false
                headerView.txtCSize.isUserInteractionEnabled = false
                headerView.txtAntiBiotic.isUserInteractionEnabled = false
            }
            ...
            handleAddCompletionSetPEHeaderDayOfAge(headerView)
            handleMinusCompletionSetPEHeaderDayOfAge(headerView)
            handleDTypeCompletionSetPEHeaderDayOfAge(headerView)
            handleCSizeCompletionSetPEHeaderDayOfAge(headerView)
            return headerView
        } else {
    ...
    fileprivate func handleAddCompletionSetPEHeaderDayOfAgeS(_ headerView: PEHeaderDayOfAge) {
        headerView.addCompletion = {[unowned self] ( error) in
    ...
    fileprivate func handleMinusCompletionSetPEHeaderDayOfAgeS(_ headerView: PEHeaderDayOfAge) {
        headerView.minusCompletion = {[unowned self] (error) in
    ...
    fileprivate func handleDTypeCompletionetPEHeaderDayOfAgeS(_ headerView: PEHeaderDayOfAge) {
        headerView.dTypeCompletion = {[unowned self] ( error) in
            let bagSizeDetailsArray = CoreDataHandlerPE().fetchDetailsFor(entityName: "PE_DOADiluentType")
            let bagSizeArray = bagSizeDetailsArray.value(forKey: "diluentName") as? NSArray ?? NSArray()
            if bagSizeArray.count > 0 {
    ...
    fileprivate func handlecSizeCompletionSetPEHeaderDayOfAgeS(_ headerView: PEHeaderDayOfAge, _ section: Int) {
        headerView.cSizeCompletion = {[unowned self] ( error) in
            let bagSizeDetailsArray = CoreDataHandlerPE().fetchDetailsFor(entityName: "PE_DOASizes")
            let bagSizeArray = bagSizeDetailsArray.value(forKey: "size") as? NSArray ?? NSArray()
            if bagSizeArray.count > 0 {
    ...
    func setPEHeaderDayOfAgeS(_ tableView: UITableView , section:Int) -> PEHeaderDayOfAge {
        if selectedCategory?.sequenceNoo == 1 {
            ...
            handleMinusCompletionSetPEHeaderDayOfAgeS(headerView)
            handleDTypeCompletionetPEHeaderDayOfAgeS(headerView)
            handlecSizeCompletionSetPEHeaderDayOfAgeS(headerView, section)
...
extension PEDraftAssesmentFinalize : UICollectionViewDelegate, UICollectionViewDataSource,UICollectionViewDelegateFlowLayout  {
    ...
    fileprivate func checkAndUpdateRefridgrateInLocalDB() {
        if(selectedCategory?.catName == Constants.refrigeratorNitrogenStr) {
            lblextenderMicro.isHidden = true
            extendedMicroSwitch.isHidden = true
            extendedMicroSwitch.isUserInteractionEnabled = false
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if checkNoteForEveryQuestion() {
            ...
            if catArrayForCollectionIs.count > indexPath.row {
                ...
                checkAndUpdateRefridgrateInLocalDB()
                ...
                if(selectedCategory?.catName == Constants.extendedMicrobialStr) {
                    ...
                    self.extendedMicroSwitch.isOn = self.peNewAssessment.IsEMRequested ?? false
                    UserDefaults.standard.set(self.peNewAssessment.IsEMRequested, forKey:"ExtendedMicro")
                } else {
                    lblextenderMicro.isHidden = true
                    extendedMicroSwitch.isHidden = true
                    extendedMicroSwitch.isUserInteractionEnabled = false
    ...
    private func assessmentNoteCountValidation(assessment:PE_AssessmentInProgress?) -> Bool? {
    ...
    private func validateAssessmentnoteCountThawingTimeStr(assessment:PE_AssessmentInProgress?) -> Bool? {
        if assessment?.note?.count ?? 0 < 1 {
    ...
    private func handleAssessmentDataLoop(assessment:PE_AssessmentInProgress?) -> Bool? {
        if assessment?.assStatus == 1 && assessment?.assID == 5 {
            if let boolStatus = self.assessmentNoteCountValidation(assessment: assessment) {
                return boolStatus
            }
        }
        else if assessment?.assStatus == 1 && assessment?.assID == 9, let boolStatus = self.validateAssessmentnoteCountThawingTimeStr(assessment: assessment) {
            return boolStatus
        }
    ...
    func checkNoteForEveryQuestion() -> Bool {
        ...
        if finishingAssessment == true {
            ...
            for obj in catArrayForTableIs {
                let assessment = obj as? PE_AssessmentInProgress
                if let boolStatus = self.handleAssessmentDataLoop(assessment: assessment) {
                    return boolStatus
                }
            }
        }
    ...
    fileprivate func handleCatArrayForCollectionIsSequenceNo() {
    ...
    func chechForLastCategory() {
        let peNewAssessmentArray = CoreDataHandlerPE().getDraftOnGoingAssessmentArrayPEObject()
        ...
        if count > 0 {
            ...
            handleCatArrayForCollectionIsSequenceNo()
        }
    }
}
...
extension PEDraftAssesmentFinalize: UIImagePickerControllerDelegate , UINavigationControllerDelegate{
    ...
    fileprivate func handleRefridgratorCellBtnImageCountValidation(_ Newassessment: PE_AssessmentInProgress) {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey: Any]) {
        ...
        if let pickedImage: UIImage = (info[convertFromUIImagePickerControllerInfoKey(UIImagePickerController.InfoKey.originalImage)]) as? UIImage {
            ...
            var Newassessment = self.catArrayForTableIs[tableviewIndexPath.row] as? PE_AssessmentInProgress ?? PE_AssessmentInProgress()
            if(selectedCategory?.sequenceNoo == 11) {
            ...
            handleRefridgratorCellBtnImageCountValidation(Newassessment)
        }
