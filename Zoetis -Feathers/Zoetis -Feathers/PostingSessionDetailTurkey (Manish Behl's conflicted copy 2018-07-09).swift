...
class PostingSessionDetailTurkey: UIViewController, turkeyNotes, UITextFieldDelegate, UIPickerViewDataSource, UIPickerViewDelegate, SyncApiDataTurkey {
    var height = false
...
let nameText = UITextField(frame: CGRect(x: 40, y: 88, width: 420, height: 50))
...
override func viewDidLoad() {
    ...
    notesBtnOutlet.setTitle(posting.notes, for: UIControlState())
...
@IBAction func notesBtnAction(_ sender: UIButton) {
    ...
    notesBGbtn.setTitle("", for: UIControlState())
...
func notesButtonAcn(_ sender: UIButton!) {
...
func showUpdateView(sender: Int) {
    ...
    buttonEdit.setTitleColor(UIColor.blue, for: UIControlState())
    ...
    farmWeightText.leftViewMode = UITextFieldViewMode.always
    ...
    let strFarmWeight = (farmArray.object(at: sender) as AnyObject).value(forKey: "farmWeight") as? String
    ...
    ageButton.titleEdgeInsets = UIEdgeInsets(top: 5.0, left: 10.0, bottom: 0.0, right: 0.0)
    ...
    feedButton.contentEdgeInsets = UIEdgeInsets(top: 0, left: 10, bottom: 0, right: 0)
    //print(strFedd)
    feedButton.setTitle(strFarmNameFeedId, for: .normal)
...
//    if farmWeightText.text?.count == 0 {
//        
...
//        
//        farmWeightText.layer.borderColor = UIColor.red.cgColor
//    }
//    
//    
//

        if farmWeightText.text?.count == 1 && farmWeightText.text == "." || farmWeightText.text?.count == 0 {

        Helper.showAlertMessage(self, titleStr: NSLocalizedString("Alert", comment: ""), messageStr: "Fields marked as (*) are mandatory. Please fill all the fields.")

        farmWeightText.layer.borderColor = UIColor.red.cgColor

        farmWeightText.text = nil
    } else {
        ...
        if trimmedString == "" {
            ...
            farmWeightText.layer.borderColor = UIColor.black.cgColor
            let alertController = UIAlertController(title: "", message: NSLocalizedString("Please enter farm name.", comment: ""), preferredStyle: UIAlertControllerStyle.alert) //Replace
            ...
            if strFeddUpdate == ""{
                ...
                CoreDataHandlerTurkey().updateFeddProgramInStep1UsingFarmNameTurkey(self.postingId, feedname: strFarmNameFeedId, feedId: feedIdUpdate, formName: strfarmName)
            ...
            CoreDataHandlerTurkey().updateNecropsystep1WithNecIdAndFarmNameTurkey(postingId, farmName: oldFarmName as NSString, newFarmName: strNewFarm as NSString, age: (ageButton.titleLabel?.text!)!, isSync: true, farmWeight: farmWeightText.text!)
                                        ...
                                        for  j in 0..<(feedDetailArr! as AnyObject).count {
                                            ...
                                            if feedCatName == "Coccidiosis Control"{
                                                ...
                                                for  m in 0..<(feedDetail! as AnyObject).count {
                                                    ...
                                                    CoreDataHandlerTurkey().getDataFromCocoiiControllForSingleDataTurkey(postDict as! NSDictionary, feedId: feedId as NSNumber, postingId: seesionId as NSNumber, feedProgramName: feedName as! String, postingIdCocoii: self.postingId)
                                                ...
                                                for  p in 0..<(feedDetail! as AnyObject).count {
                                                    ...
                                                    CoreDataHandlerTurkey().getDataFromAlterNativeForSingleDevTokenTurkey(postDict as! NSDictionary, feedId: feedId as NSNumber, postingId: seesionId as NSNumber, feedProgramName: feedName as! String, postingAlterNative: self.postingId)
                                                ...
                                                for  a in 0..<(feedDetail! as AnyObject).count {
                                                    ...
                                                    CoreDataHandlerTurkey().getDataFromAntiboiticWithSigleDataTurkey(postDict as! NSDictionary, feedId: feedId as NSNumber, postingId: seesionId as NSNumber, feedProgramName: feedName as! String, postingIdAlterNative: self.postingId)
                                                ...
                                                CoreDataHandlerTurkey().getDataFromMyCocotinBinderWithSingleDataTurkey(postDict as! NSDictionary, feedId: feedId as NSNumber, postingId: seesionId as NSNumber, feedProgramName: feedName as! String, postingidMycotxin: self.postingId)
...
func alerViewInternet() {
    let alertController = UIAlertController(title: "", message: NSLocalizedString("No internet connection. Please try again!", comment: ""), preferredStyle: UIAlertControllerStyle.alert) //Replace
    let okAction = UIAlertAction(title: NSLocalizedString("Ok", comment: ""), style: UIAlertActionStyle.default) {
        (_: UIAlertAction) -> Void in
        Helper.dismissGlobalHUD(self.view)
    }
...
func alerViewSucees() {
    let alertController = UIAlertController(title: NSLocalizedString("Alert", comment: ""), message: NSLocalizedString("Data sync has been completed.", comment: ""), preferredStyle: UIAlertControllerStyle.alert) //Replace
    let okAction = UIAlertAction(title: NSLocalizedString("Ok", comment: ""), style: UIAlertActionStyle.default) {
        (_: UIAlertAction) -> Void in
        self.navigationController?.popViewController(animated: true)
    }
...
@IBAction func reciveButton(_ sender: Any) {
    ...
    if (postingSess == true || necropcySess == true) {
        let alertController = UIAlertController(title: NSLocalizedString("Alert", comment: ""), message: "Unsynced data available for this posting session. Please sync and then try again else your local data will be lost. Do you want to continue with the request?", preferredStyle: .alert)
...
@IBAction func syncToWebAction(_ sender: UIButton) {
    ...
    if self.allSessionArrWithPostingId(PId: postingId).count > 0 {
        ...
        Helper.showAlertMessage(self, titleStr: NSLocalizedString("Alert", comment: ""), messageStr: "Data not available for syncing.")
    }
}
...
func clickImage(_ sender: UIButton) {
...
func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    ...
    if tableView == droperTableView {
        ...
        cell.farmNameLbl.text = farmArrayWithoutAge!  + " " + "[" + age + "]"
        ...
        if sick == 0 {
            cell.sickLbl.text = "No"
        } else if sick == 1 {
            cell.sickLbl.text = "Yes"
        }
     ...
     let lastVarble = farmWeight?.last
        ...
        cell.badgeButton.badgeEdgeInsets = UIEdgeInsets(top: 10, left: 0, bottom: 0, right: 15)
        ...
        cell.badgeButton.setImage(UIImage(named: "galaryBlue"), for: UIControlState())
        } else {

        cell.badgeButton.setImage(UIImage(named: "galary"), for: UIControlState())
        }

//        if height == true {

//            cell.farmNameLbl.frame.size.height = 80
//            cell.feedProgramLbl.frame.size.height = 80
//            cell.farmWeightLbl.frame.size.height = 80
//            cell.noOfBirdsLbl.frame.size.height = 80
//            cell.breedLbl.frame.size.height = 80
//            cell.sexLbl.frame.size.height = 80
//            cell.abfLbl.frame.size.height = 80
//            cell.flockIdLbl.frame.size.height = 80
//            cell.houseNoLbl.frame.size.height = 80
//            cell.sickLbl.frame.size.height = 80
//            cell.badgeButton = MIBadgeButton(frame:CGRect(x: 144, y: 80, width: 31, height: 30))
//            cell.updateButton = UIButton(frame:CGRect(x: 144, y: 80, width: 31, height: 30))
//
//        }else {
//
//            cell.farmNameLbl.frame.size.height = 115
//            cell.feedProgramLbl.frame.size.height = 115
//            cell.farmWeightLbl.frame.size.height = 115
//            cell.noOfBirdsLbl.frame.size.height = 115
//            cell.breedLbl.frame.size.height = 115
//            cell.sexLbl.frame.size.height = 115
//            cell.abfLbl.frame.size.height = 115
//            cell.flockIdLbl.frame.size.height = 115
//            cell.houseNoLbl.frame.size.height = 115
//            cell.sickLbl.frame.size.height = 115
//            cell.badgeButton.frame.size.height = 80
//            cell.updateButton.frame.size.height = 80
//
//
//            cell.badgeButton = MIBadgeButton(frame:CGRect(x: 144, y: 30, width: 31, height: 30))
//
//            cell.updateButton = UIButton(frame:CGRect(x: 144, y: 30, width: 31, height: 30))
//        }
        return cell
    }
}
