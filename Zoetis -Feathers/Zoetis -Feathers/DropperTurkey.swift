...
typealias WSCompletionBlockT = (_ responseData: RouteTurkey?) ->Void
...
class DropperTurkey: UIView {
    ...
    override  func layoutSubviews() {
        ...
        // Styling
        TableMenu.backgroundColor = UIColor.black
    ...
    func showWithAnimationWithBlocks(_ time: TimeInterval, options: Alignment, position: Position = .bottom, button: UIButton, newItems: NSArray, completionBlock:@escaping WSCompletionBlockT) {
...
//        if UserDefaults.standard.bool(forKey: "Chicken") == true {
//
//            let skeletaObject : Route = items[indexPath.row] as! Route
//
//
//        } else if UserDefaults.standard.bool(forKey: "turkey") == true {
//

            let skeletaObject: RouteTurkey = items[indexPath.row] as! RouteTurkey
    //    }

        let item = skeletaObject.routeName
