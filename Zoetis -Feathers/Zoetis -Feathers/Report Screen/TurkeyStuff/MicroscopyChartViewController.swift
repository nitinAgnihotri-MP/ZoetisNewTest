...
//        }
    ...
    func callCommonFunction(_ catName: NSString) {
        ...
        if arrayOfIds.count > 2 {
            ...
            for i in ((arrayOfIds.count)-2...(arrayOfIds.count)).reversed() {
                ...
                let objectArray: NSMutableArray =  CoreDataHandlerTurkey().fetchAllPostingSessionTurkey(arrayOfIds[i-1] as! NSNumber).mutableCopy() as! NSMutableArray
    ...
    @IBAction func BtnByWeekPressed(_ sender: UIButton) {
       ...
       // lineChartView.xAxis.valueFormatter = formatter
        ...
        let arrayOfIds = AllValidSessions.sharedInstance.allValidSession as NSArray
        let objectArray1 =  CoreDataHandlerTurkey().fetchAllPostingSessionTurkey(arrayOfIds[arrayOfIds.count-1] as! NSNumber).mutableCopy() as! NSMutableArray
