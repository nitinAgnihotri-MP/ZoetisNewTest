...
class Report_MainVCViewController: UIViewController, GI_TtactDelegate, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        if sessionDataArray.count > 0 {
           ...
           for i in 0..<sessionDataArray.count {
            ...
            var sortedResults = sessionDat.sortedArray(using: [descriptor]) as NSArray
            sortedResults = sortedResults.reversed() as NSArray
            sessionDataArray = sortedResults
        }
        for i in 0..<sessionDataArray.count {
    ...
    @IBAction func btn_Skeletal_Action(_ sender: AnyObject) {
        ...
        verticalValues = observationNameCrop(values: verticalValues) as! [String]
        self.headerTitle = "Skeletal/Muscular/Integumentary"
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let descriptor: NSSortDescriptor = NSSortDescriptor(key: "sessiondate", ascending: false)
        var sortedResults = sessionDataArray.sortedArray(using: [descriptor]) as NSArray
        sortedResults = sortedResults.reversed() as NSArray
        sessionDataArray = sortedResults

        for i in 0..<sortedResults.count {
            if(((sortedResults.object(at: i) as AnyObject).value(forKey: "lngId")!) as! Int == Regions.languageID) {
                AllValidSessions.sharedInstance.allValidSession.add((sortedResults.object(at: i) as AnyObject).value(forKey: "postingId")!)
            }
        }
