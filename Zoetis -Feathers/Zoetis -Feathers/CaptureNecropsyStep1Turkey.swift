...
//
        ...
        } else  if (feedProgramDisplayLabel.text == NSLocalizedString("- Select -", comment: "") ||  ageLbl.text == "" || farmNameTextfield.text == "" )//|| farmWeightTextField.text == "")
        {
...
//        let a = CoreDataHandlerTurkey().FetchNecropsystep1NecIdTurkey(neciIdStep as NSNumber)
//        if a.count > 0{
//            let b = a.lastObject as! CaptureNecropsyDataTurkey
//            var farmName = b.farmName
//            if (farmName?.contains("."))! {
//                let endIndex = farmName?.range(of: ".")!.lowerBound
//                farmName = farmName?.substring(to: endIndex!).trimmingCharacters(in: .whitespacesAndNewlines)
//            }
//            let c = Int(farmName!)
//            count = c!+1
//
...
//        }
       ...
       // let appendfeedProgramwithCount = String(format:"%d. %@ [%@]",count,trimmedString,self.ageLbl.text!)
        ...
        print(appendfeedProgramwithCount)
        self.timeStamp()
        if farmWeightTextField.text == "."{
            farmWeightTextField.text = ""
        }

        CoreDataHandlerTurkey().SaveNecropsystep1Turkey(neciIdStep as NSNumber, age: self.ageLbl.text!, farmName: appendfeedProgramwithCount, feedProgram: feedProgramDisplayLabel.text!, flockId: flockIdTextField.text!, houseNo: houseNoLBL.text!, noOfBirds: noOfBirdsLbl.text!, sick: asb as NSNumber, necId: neciIdStep as NSNumber, compexName: complexLbl.text!, complexDate: postingDateLbl.text!, complexId: complexId as NSNumber, custmerId: custMid as NSNumber, feedId: feedId as NSNumber, isSync: true, timeStamp: timeStampString, actualTimeStamp: timeStampString, lngId: lngId as NSNumber, farmWeight: farmWeightTextField.text!, abf: abfLbl.text!, breed: breedString, sex: sexString, farmId: countFarmId as NSNumber, imageId: NSNumber(value: imageAutoIncrementId), count: count as NSNumber)
    ...
    func timeStamp() -> String {
        if UserDefaults.standard.bool(forKey: "Unlinked") == true {
            ...
            let string = timeStampString as String!
   ...
   //*********
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        } else if ((textField.text?.characters.count)! > 49  ) {
            return false
        }
