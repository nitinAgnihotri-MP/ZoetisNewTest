...
class PVEStartNewAssFinalizeAssement: BaseViewController  , UISearchBarDelegate {
    ...
    @IBAction func draftBtnAction(_ sender: Any) {
        let alertController = UIAlertController(title: "Alert", message: "Are you sure you want to save assessment in draft?", preferredStyle: .alert)
    ...
    @IBAction func infoBtnAction(_ sender: UIButton) {
        ...
        var newAssessmentsArr = questionsArr.value(forKey: "assessment") as? [String]
        var assessment = newAssessmentsArr![indexPath!.row]
        ...
        if currentSel_seq_Number == 2 {
            ...
            if selectedIndex == 4{
                ...
                newAssessmentsArr = liveQuesArr.value(forKey: "assessment") as? [String]
                assessment = newAssessmentsArr![indexPath!.row]
                ...
                newAssessmentsArr = inactiveQuessArr.value(forKey: "assessment") as? [String]
                assessment = newAssessmentsArr![indexPath!.row]
            }
            else{
                ...
                newAssessmentsArr = otherQuessArr.value(forKey: "assessment") as? [String]
                assessment = newAssessmentsArr![indexPath!.row]
            }
        }
        else{
            ...
            newAssessmentsArr = questionsArr.value(forKey: "assessment") as? [String]
            assessment = newAssessmentsArr![indexPath!.row]
...
extension PVEStartNewAssFinalizeAssement:  UIImagePickerControllerDelegate,UINavigationControllerDelegate{
    ...
    fileprivate func handleImagePicker(_ indexPath: IndexPath?) {
    ...
    func getImageDataArr(currentSelectedBtnIndex:Int) -> [Data]{
        ...
        var tempArr = CoreDataHandlerPVE().getImageDataForCurrentAssementDetails("", seqNumber: NSNumber(value: seq_Number), rowId: id, entity: "PVE_ImageEntity")
...
extension PVEStartNewAssFinalizeAssement: VaccinatorsPlusBtnDelegate,NoOfvaccinatorsMinusDelegate {
    ...
    fileprivate func resetAndInsertVaccinatorRows(_ count: Int) {
        if noOfVaccinatorsArr.count > 0{
    ...
    func vaccinatorPlusBtnTapped(count: Int) {
        ...
        if count != 0 {
            ...
            if count < noOfVaccinatorsArr.count && noOfVaccinatorsArr.count > 0{
                ...
                resetAndInsertVaccinatorRows(count)
            }
...
extension PVEStartNewAssFinalizeAssement: NoOfCatchersMinusDelegate,CatchersPlusBtnDelegate , AddedComment{
    ...
    fileprivate func handlednoOfCatcherArrAndInsertDelete(_ count: Int) {
    ...
    func catchersbtnPlusBtnTapped(count: Int) {
        ...
        if count != 0 {
            ...
            handlednoOfCatcherArrAndInsertDelete(count)
...
extension PVEStartNewAssFinalizeAssement: VaccinatorInfoDetailPlusBtnTapped,VaccineInfoDetailsMinusBtnDelegate  {
    ...
    fileprivate func handleTextFieldValidation(_ cell: PVEVaccineInfoDetailsCell) {
        ...
        for (indx, _) in vaccinInfoDetailArr.enumerated() {
            ...
            if let cell = self.tblView.cellForRow(at: indexPath) as? PVEVaccineInfoDetailsCell,
               cell.vacManTxtFld.text! == "" || cell.vacNameTxtFld.text! == "" || cell.serotypeTxtFld.text! == "" {
                handleTextFieldValidation(cell)
            }
    ...
    fileprivate func saveAndupdateAssessmentDataInLocalDB(_ idArr: [Int]?, _ seqArr: [Int]?) {
    ...
    @objc func switchTapped(sender:UISwitch) {
        if sender.tag == 4 {
            ...
            saveAndupdateAssessmentDataInLocalDB(idArr, seqArr)
...
extension PVEStartNewAssFinalizeAssement: UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        switch currentSel_seq_Number {
        case 2:
            return heightForVaccinePreparationSection(at: indexPath)
        case 6:
            return 1350.0
        default:
            return 80.0
    ...
    private func heightForVaccinePreparationSection(at indexPath: IndexPath) -> CGFloat {
        ...
        case 4, 5:
            return 80.0
        case 6:
            return heightForVaccinInfoDetail(at: indexPath.row)
        default:
            return 80.0
        }
    }

    private func heightForVaccinInfoDetail(at row: Int) -> CGFloat {
        guard let showMoreValue = vaccinInfoDetailArr[row]["showMore"] as? String else {
            return 300.0
        }
        return showMoreValue == "No" ? 93.0 : 300.0
    }
    
    private func configureVaccineInfoTypeCell(for indexPath: IndexPath,tableView:UITableView) -> UITableViewCell {
    ...
    private func configureCatchersCell(for indexPath: IndexPath,tableView:UITableView) -> UITableViewCell {
	...
	fileprivate func handleNoOfVaccinatorsValidation(_ indexPath: IndexPath, _ cell: PVETeamMemeberVaccinatorsCell) {
	...
	private func configureVaccinatorsCell(for indexPath: IndexPath,tableView:UITableView) -> UITableViewCell {
        ...
        if noOfVaccinatorsArr.count > indexPath.row{
			...
			handleNoOfVaccinatorsValidation(indexPath, cell)
        } else {
            cell.nameTxtField.text = ""
            cell.mobileTxtField.text = ""
            cell.emailTxtField.text = ""
    ...
    private func configureCrewSafetyCell(for indexPath: IndexPath,tableView:UITableView) -> UITableViewCell {
    ...
    private func configureVaccinationCellAppearance(cell: PVEVaccinationCrewSafetyCell, indexPath: IndexPath) {
        ...
        let cameraState = sharedManager.getSessionValueForKeyFromDB(key: "cameraEnabled") as! String
        let cameraAlpha: CGFloat = (cameraState == "true") ? 1.0 : 0.2
        cell.cameraIcon.alpha = cameraAlpha
        cell.cameraBtn.alpha = cameraAlpha
	...
	fileprivate func handleCellVacManTxtFld(_ indexPath: IndexPath, _ cell: PVEVaccineInfoDetailsCell) {
    ...
    fileprivate func handleVaccineInfoDetailSubMethod1(_ indexPath: IndexPath, _ cell: PVEVaccineInfoDetailsCell) {
        if vaccinInfoDetailArr.count > indexPath.row {
            ...
            handleVaccineInfDetailsOtherValidations(indexPath, cell)
    ...
    private func configureIsInActiveVaccineOnCell(for indexPath: IndexPath,tableView:UITableView) -> UITableViewCell {
        ...
        handleVaccineInfoDetailSubMethod1(indexPath, cell)
        ...
        if let expDate = vaccinInfoDetailArr[indexPath.row]["expDate"] as? String, !expDate.isEmpty {
            self.sharedManager.setBorderBlue(btn: cell.expiryBtn)
        }

        if let siteOfInj = vaccinInfoDetailArr[indexPath.row]["siteOfInj"] as? String, !siteOfInj.isEmpty {
    ...
    fileprivate func handleCurrentSelSeqNumber6(_ tableView: UITableView, _ indexPath: IndexPath) -> UITableViewCell {
    ...
    fileprivate func handleCameraStateValidation(_ tableView: UITableView, _ indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "PVEVaccinationCrewSafetyCell", for: indexPath) as! PVEVaccinationCrewSafetyCell
        cell.setCellAndControllsNew(qArr:otherQuessArr, currentIndd: indexPath as NSIndexPath)
        cell.tag = Int("\(indexPath.section)" + "\(indexPath.row)")!
        let cameraState = sharedManager.getSessionValueForKeyFromDB(key: "cameraEnabled") as! String
    ...
    fileprivate func handleCameraStateValidationElseStateement(_ tableView: UITableView, _ indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "PVEVaccinationCrewSafetyCell", for: indexPath) as! PVEVaccinationCrewSafetyCell
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if currentSel_seq_Number == 2 {
            if indexPath.section == 0 {
                return configureVaccineInfoTypeCell(for: indexPath, tableView: tableView)
            } else if indexPath.section == 1 {
                return configureCatchersCell(for: indexPath,tableView:tableView)
            } else if indexPath.section == 2 {
                return configureVaccinatorsCell(for: indexPath, tableView: tableView)
            } else if indexPath.section == 3 {
                return configureCrewSafetyCell(for: indexPath, tableView: tableView)
            } else if indexPath.section == 4 {
                return configureVaccineNoteCell(for: indexPath, tableView: tableView)
            } else if indexPath.section == 5 {
                return configureVaccineDetailsCell(for: indexPath, tableView: tableView)
            } else if indexPath.section == 6 {
                return configureIsInActiveVaccineOnCell(for: indexPath, tableView: tableView)
            } else {
                return handleCameraStateValidation(tableView, indexPath)
            }
        } else if currentSel_seq_Number == 6 {
            return handleCurrentSelSeqNumber6(tableView, indexPath)
            
        } else {
            return handleCameraStateValidationElseStateement(tableView, indexPath)
        }
    }
    ...
    func antigenFunc (selectedRow : Int) {
        ...
        if let items = vaccinInfoDetailArr[selectedRow]["serotype"] as? [String],
           let itemsIds = vaccinInfoDetailArr[selectedRow]["serotype_id"] as? [String] {
            ...
            for id in 0..<items.count {
                if let idInt = Int(itemsIds[id]) {
                    let item = SelectedData(id: idInt, name: items[id])
                    selectedAntigenArray.append(item)
                }
            }
        }
    ...
    func tableHeaderViewSection2(tableView:UITableView) -> NoOfVaccinatorsHeader {
    ...
    fileprivate func tableHeaderViewSection6(_ tableView: UITableView) -> UIView? {
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        if section == 2 {
            return self.tableHeaderViewSection2(tableView: tableView)
        }
        ...
        if section == 6 {
            return tableHeaderViewSection6(tableView)
        }
        if section == 3 {
...
extension PVEStartNewAssFinalizeAssement: UICollectionViewDelegate, UICollectionViewDataSource,UICollectionViewDelegateFlowLayout {
        ...
        if vaccineNamesArr.count > 0 {
            ...
            self.dropDownVIewNew(arrayData: vaccineNamesArr as! [String], kWidth: sender.frame.width, kAnchor: sender, yheight: sender.bounds.height) { [unowned self] selectedVal, index in
                ...
                if let cell = self.tblView.cellForRow(at: currentIndPath!) as? PVEVaccineInfoDetailsCell,
                   currentIndPath! == cell.currentIndPath as IndexPath {
    ...
    fileprivate func updateCoredataIfSelectedAntigenId37(_ currentIndPath: IndexPath?, _ cell: PVEVaccineInfoDetailsCell) {
    ...
    fileprivate func updateSerotypeSelectionUIAndData(_ vaccineNamesIdArr: NSArray,_ currentIndPath: IndexPath?,_ cell: PVEVaccineInfoDetailsCell, _ selectedVal: String, _ index: Int) {
        if currentIndPath! == cell.currentIndPath as IndexPath {
    ...
    @IBAction func serotypeBtnAction(_ sender: UIButton) {
        ...
        if vaccineNamesArr.count > 0 {
            ...
            selectionMenu.setSelectedItems(items: simpleSelectedArray) { [self] (name, index, selected, selectedItems) in
                ...
                if let cell = self.tblView.cellForRow(at: currentIndPath!) as? PVEVaccineInfoDetailsCell {
                    self.updateCoredataIfSelectedAntigenId37(currentIndPath, cell)
                }
            }
            self.tblView.reloadRows(at: [currentIndPath!], with: .none)
            
        } else {
            let vaccineManArr = CoreDataHandlerPVE().fetchDetailsForEntity(entityName: "PVE_SerotypeDetails", id:  0 , keyStr: "vaccine_Id")
            let vaccineNamesArr = vaccineManArr.value(forKey: "type") as? NSArray ?? NSArray()
            let vaccineNamesIdArr = vaccineManArr.value(forKey: "id") as? NSArray ?? NSArray()
            if vaccineNamesArr.count > 0 {
                ...
                self.dropDownVIewNew(arrayData: vaccineNamesArr as! [String], kWidth: sender.frame.width, kAnchor: sender, yheight: sender.bounds.height) { [unowned self] selectedVal, index in
                    ...
                    if let cell = self.tblView.cellForRow(at: currentIndPath!) as? PVEVaccineInfoDetailsCell {
                        updateSerotypeSelectionUIAndData(vaccineNamesIdArr,currentIndPath,cell, selectedVal, index)
                    }
                    self.tblView.reloadData()
                }
                self.dropHiddenAndShow()
...
extension PVEStartNewAssFinalizeAssement: DatePickerPopupViewControllerProtocol{
    ...
    func doneButtonTappedWithDate(string: String, objDate: Date) {
        if let cell = self.tblView.cellForRow(at: clickedDateIndpath as IndexPath) as? PVEVaccineInfoDetailsCell,
           clickedDateIndpath == cell.currentIndPath {
